...
from data.recettes_archeologue import RECETTES_ARCHEOLOGUE

archeologue_group = app_commands.Group(name="archeologue", description="Actions du métier d'Archéologue")
...
DECOUVERTES = [
    "une inscription à moitié effacée sur un mur de ruines",
    "un texte gravé dans une grotte oubliée",
    "les restes d'un ancien campement de fouille",
    "une stèle couverte de mousse près d'un temple abandonné",
    "des marques mystérieuses sur un rocher isolé",
]

@archeologue_group.command(name="etudier", description="Étudier des ruines pour gagner des Berrys (réservé aux Archéologues)")
@require_salon("salon_taverne")
async def archeologue_etudier(interaction: discord.Interaction):
    ...
    if player is None:
        ...
        return
    if player["metier"] != "Archéologue":
        await interaction.followup.send("⛔ Tu dois être **Archéologue** pour étudier des ruines ! Choisis ce métier avec `/metier choisir` (réservé aux Civils).")
        return
    ...
    last = _last_etudier.get(key)
    if last:
        elapsed = (now - last).total_seconds() / 60
        if elapsed < cooldown_minutes:
            restant = int(cooldown_minutes - elapsed)
            await interaction.followup.send(f"😮‍💨 Tu dois encore attendre **{restant} min** avant de retourner étudier.")
            return
    ...
    _last_etudier[key] = now
    niveaux_gagnes, nouveau_niveau = await add_xp(interaction.guild_id, interaction.user.id, 8, 4)
    await increment_quest_progress(interaction.guild_id, interaction.user.id, "etudier_ruines")

    embed = discord.Embed(
        title="🏺 Ruines étudiées !",
        description=f"Tu déchiffres {decouverte} et revends tes découvertes à un érudit local pour **{gain}฿** !",
        color=0xD4A017
    )
...
async def recette_archeologue_autocomplete(interaction: discord.Interaction, current: str):
    player = await get_player(interaction.guild_id, interaction.user.id)
    if not player or player["metier"] != "Archéologue":
        return []
    rang = get_rang(player["metier_xp"])
    dispo = [r for r in RECETTES_ARCHEOLOGUE if r[3] <= rang and current.lower() in r[0].lower()]
    return [app_commands.Choice(name=r[0], value=r[0]) for r in dispo[:25]]

@archeologue_group.command(name="dechiffrer", description="Déchiffrer une relique à partir de vestiges anciens (réservé aux Archéologues)")
@app_commands.describe(recette="La relique à déchiffrer")
@app_commands.autocomplete(recette=recette_archeologue_autocomplete)
@require_salon("salon_taverne")
async def archeologue_dechiffrer(interaction: discord.Interaction, recette: str):
    await interaction.response.defer()
    player = await get_player(interaction.guild_id, interaction.user.id)
    if player is None:
        await interaction.followup.send("Tu n'as pas encore de personnage ! Lance `/commencer` pour débuter l'aventure 🏴‍☠️")
        return
    if player["metier"] != "Archéologue":
        await interaction.followup.send("⛔ Tu dois être **Archéologue** pour déchiffrer une relique ! Choisis ce métier avec `/metier choisir` (réservé aux Civils).")
        return

    recette_data = next((r for r in RECETTES_ARCHEOLOGUE if r[0] == recette), None)
    if not recette_data:
        await interaction.followup.send("Recette introuvable.")
        return
    nom_recette, ingredients, nom_relique, rang_requis = recette_data
    ...
    async with pool.acquire() as conn:
        ...
        relique_item = await get_item_by_name(interaction.guild_id, nom_relique)
        if not relique_item:
            await interaction.followup.send("⚠️ Erreur interne : la relique n'existe pas dans le catalogue.")
            return
        ...
        async with conn.transaction():
            ...
            existing = await conn.fetchrow(
                ...
                interaction.guild_id, interaction.user.id, relique_item["id"]
            )
            if existing:
                ...
                await conn.execute(
                    ...
                    interaction.guild_id, interaction.user.id, relique_item["id"], relique_item["durabilite_max"]
                )
    ...
    niveaux_gagnes, nouveau_niveau = await add_xp(interaction.guild_id, interaction.user.id, 8, 4)
    await increment_quest_progress(interaction.guild_id, interaction.user.id, "dechiffrer_relique")
    ...
    embed = discord.Embed(
        title="🏺 Relique déchiffrée !",
        description=f"Tu as déchiffré **{nom_relique}** ! Retrouve-la dans `/inventaire voir`.",
        color=0xF4C430
    )
    ...
    if rang_monte:
        await interaction.followup.send(embed=discord.Embed(
            ...
            description=f"Tu passes rang **{rang_label(nouveau_rang)}** en tant qu'Archéologue !",
            color=0x27AE60
        ))
    if niveaux_gagnes > 0:
        await interaction.followup.send(embed=discord.Embed(
...
def setup_archeologue_commands(bot):
    bot.tree.add_command(archeologue_group)
